    ...
    return (
        <div className={cx('post', { divide })}>
            <div className={cx('image')} style={{ backgroundImage: `url(${data.imageUrl})` }} >
                <Link to={`/posts/${data.path}`} onClick={onClick}>
            ...
            <div className={cx('post-text')}>
                <Link to={`/posts/${data.path}`} className={cx('post-title')} onClick={onClick}>{data.title}</Link>
                ...
                <Link to={`/posts/${data.path}`} className={cx('see-more-btn')} onClick={onClick}>Đọc thêm <span><RiArrowRightUpLine /></span></Link>
            </div>
        </div>
    )
}
